...
print(f"[DEBUG] USE_CLOUDFLARE_EMAIL = {USE_CLOUDFLARE_EMAIL}")
print(f"[DEBUG] CLOUDFLARE_EMAIL_API = {CLOUDFLARE_EMAIL_API}")
if USE_CLOUDFLARE_EMAIL:
    try:
        ...
        print(f"[DEBUG] CF_EMAIL_SERVICE = {CF_EMAIL_SERVICE}")
    except Exception as e:
    ...
    print(f"⚠️ Cloudflare 邮箱未启用，USE_CLOUDFLARE_EMAIL = {USE_CLOUDFLARE_EMAIL}")
...
def _get_mailtm_domain():
    """获取 Mail.tm 可用域名"""
    try:
        session = _create_mailtm_session()
        res = session.get(f"{MAILTM_API_BASE}/domains", timeout=15, impersonate="chrome131")
...
def _fetch_email_detail_mailtm(mail_token: str, msg_id: str):
    """获取 Mail.tm 单封邮件详情"""
    try:
        headers = {"Authorization": f"Bearer {mail_token}"}
        session = _create_mailtm_session()
...
# Debug mode: set DEBUG=1 env var to print response bodies in logs
...
class ChatGPTRegister:
    ...
    def _create_mailtm_session(self):
        """创建带重试的 Mail.tm 请求会话"""
        session = curl_requests.Session()
        session.headers.update({
            "User-Agent": self.ua,
    ...
    def _fetch_emails_mailtm(self, mail_token: str):
        ...
        try:
            ...
            self._print(f"[Mail.tm] fetch emails error: {e}")
            return []

    def _fetch_email_detail_mailtm(self, mail_token: str, msg_id: str):
        """获取 Mail.tm 单封邮件详情（直连，不走代理）"""
        try:
            import urllib.request, json as _json
            if isinstance(msg_id, str) and msg_id.startswith("/messages/"):
                msg_id = msg_id.split("/")[-1]
            ...
            self._print(f"[Mail.tm] fetch detail error: {e}")
        return None
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, mail_token: str = None):
        ...
        if need_oauth_otp:
            ...
            while time.time() < otp_deadline and not otp_success:
                messages = self._fetch_emails_mailtm(mail_token) or []
                candidate_codes = []
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    print(f"\n{'#'*60}")
    print(f"  ChatGPT 批量自动注册 (Mail.tm 临时邮箱版)")
    print(f"  注册数量: {total_accounts} | 并发数: {actual_workers}")
    print(f"  Mail.tm: {MAILTM_API_BASE}")
...
def main():
    ...
    parser.add_argument('--once', action='store_true', help='注册一个账号后退出')
    ...
    # 交互式模式
    print("=" * 60)
    print("  ChatGPT 批量自动注册工具 (Mail.tm 临时邮箱版)")
    print("=" * 60)
